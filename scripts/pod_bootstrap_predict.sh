...
#
# EVERYTHING this pod writes goes under RESULTS_DIR — its log, the unpacked bundle, m1x,
# derived, models, ledger, reports. The volume root is the DataAcquistion repo's (data*/, m1/,
# its own code/ and _pod_logs/) and other projects keep their own results/<name>/: read the
# root, never write it. Same prefix as RESULTS_PREFIX in scripts/_common.sh and src/config.py.
set +e
export RESULTS_DIR="/workspace/results/InvestOpediaClaude"
...
if [ -f "$MARKER" ]; then
  ...
  WORKDIR="$RESULTS_DIR/code/predict/src_${JOB:-stage1}"   # per-job dir: concurrent pods share the volume
  rm -rf "$WORKDIR" && mkdir -p "$WORKDIR" && cd "$WORKDIR"
  if tar xzf "$RESULTS_DIR/code/predict/bundle.tgz" --no-same-owner -m; then
    ...
    export M1_DIR="${M1_DIR:-/workspace/m1}"            # DataAcquistion's — read only
    export EOD_DIR="${EOD_DIR:-/workspace/data}"        # DataAcquistion's — read only
    export MARKET_DIR="${MARKET_DIR:-$RESULTS_DIR/m1x}"
    ...
    # not for migrate: an OUT_DIR created under results/ before the move would collide with it
    [ "${JOB:-}" = "migrate" ] || mkdir -p "$OUT_DIR"
...
sug = os.path.join(os.environ["RESULTS_DIR"], "derived/predict/suggestions.json")
try:
...
PY
      local BUNDLE_NAME="${REPORT_BUNDLE:-latest}" OUT="$RESULTS_DIR/reports/${REPORT_BUNDLE:-latest}"
      # --volume is the mount root: the session grid is read from m1/, every stage artifact
      # from results/InvestOpediaClaude/derived/ (tools/build_reports.py VOLUME_LAYOUT)
      ...
      cp -f "$RESULTS_DIR/derived/predict/suggestions.md" "$OUT/suggestions_latest.md" 2>/dev/null || true
    }
    ...
    case "${JOB:-stage1}" in
      ...
      # one-shot move of the pre-2026-09-17 root layout into RESULTS_DIR (idempotent)
      migrate) timeout 7200 python tools/migrate_results_layout.py --root /workspace ;;
