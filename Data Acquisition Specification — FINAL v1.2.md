...
4. **§2 Sharadar endpoint** — retail keys are served by `api.sharadar.com`, not the `data.nasdaq.com` datatables API in the §2 preamble. Hard limits, none documented in v1.2: `ticker` capped at **30 tickers AND 200 chars**; JSON responses capped at **10,000 rows** (this cap MOVED from 100,000 to 10,000 between 2026-08-13 and 2026-08-14 — treat it as volatile); `offset` paging; `years=N` returns a bulk CSV zip (retail tier serves 5Y; `years=10` → 403). Rate-limit headers are `x-ratelimit-*` and `-reset` is a **UNIX timestamp**.
